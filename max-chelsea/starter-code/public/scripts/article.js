...
(function (module) {
  ...
  */
    Article.all = rawData.map(function(datum){
      new Article(datum);
    })
  ...
  Article.numWordsAll = () => {
    ...
    .reduce(function(firstNum, secondNum){
  ...
  Article.allAuthors = () => {
    ...
    .reduce(function(aggregator, authors){
      if(!aggregator.includes(authors)){
        return aggregator.push(authors);
      }
    }, [])
  };
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(author => {
      return {'author': author, 'wordCount' : Article.all
      .filter(function(articles){
        return articles.author = author;
      })
      .reduce(function(total, currentValue) {
        return currentValue.body.split(' ').length + total;
      }, 0)};
      ...
      // property.
      // gives us an array of all unique author names
